...
with col1:
    ...
    st.markdown("This section covers customization using the `height`, `theme`, `shortcuts`, and `focus` properties.")
    st.markdown("### Height")
    st.markdown("The height of the code editor can be set with the `height` argument which takes one of three types of values: a string, an integer number, or an list of two integers.")
    ...
    st.markdown("If instead, `height` is set with an integer, it will be used to set the `maxLines` property of the editor. This means that the editor will adjust its height to fit the maximum number of lines upto the integer value given.")
    st.success("**Tip:** If you want the editor to always expand to fit all of the code so that no scrolling is needed, you can set `height` to a very large integer value like 1000.")
    st.markdown("Unsurprisingly, the inner editor also has a `minLines` property. It is set to 1 by default. If you want to set the minimum number of lines, you can set `height` to an list of two integers. The first integer will be used to set `minLines` and the second integer will be used to set `maxLines`.")
    ...
    st.markdown("### Theme")
    st.markdown("As mentioned earlier, the code editor component contains an inner editor component. This inner editor is an [Ace Editor](https://ace.c9.io/) which comes with 20 built in themes. These themes were not created with streamlit's modern asthetic in mind. For better integration with streamlit's look, two custom Ace Editor themes called 'streamlit-dark' and 'streamlit-light' were created. These two themes can be used as a starting point for further customization of appearence (see Advanced Customization).")
